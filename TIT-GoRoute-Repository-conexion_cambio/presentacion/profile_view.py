...
def render_profile(user_id):
    ...
    # Identificamos si es el propio perfil del usuario logueado (necesario para pestañas y edición)
    ...
    with col_i:
        ...
        # POP-UP DE EDICIÓN MEJORADO (Solo lo ve el dueño del perfil)
        if es_mi_perfil:
    ...
    def pintar_cuadricula(rutas_lista, es_seccion_favoritos=False):
        if not rutas_lista:
            st.info("Aún no hay rutas para mostrar aquí.")
            return
        ...
        for i, ruta in enumerate(rutas_lista):
            with cols[i % 3]:
                ...
                if btn_col1.button("Ver publicación", key=f"ver_{prefijo}_{ruta['id']}", use_container_width=True):
    ...
    # RENDERIZADO DE LAS PESTAÑAS Y CUADRÍCULA
    if es_mi_perfil:
        tab_mis_rutas, tab_favs = st.tabs(["📍 Mis Rutas", "⭐ Favoritos"])
        
        with tab_mis_rutas:
            pintar_cuadricula(user["rutas"], es_seccion_favoritos=False)
            
        with tab_favs:
            rutas_favs = profile_service.get_rutas_favoritas(user_id)
            pintar_cuadricula(rutas_favs, es_seccion_favoritos=True)
    else:
        st.subheader("📍 Rutas Publicadas")
        pintar_cuadricula(user["rutas"], es_seccion_favoritos=False)