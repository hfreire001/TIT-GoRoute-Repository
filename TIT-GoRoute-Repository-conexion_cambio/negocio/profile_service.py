...
def get_rutas_favoritas(user_id):
    """Obtiene la lista de rutas favoritas del usuario."""
    return user_repo.get_favoritos_usuario(user_id)

# Asegúrate de importar user_repo si no lo tienes ya en este archivo:
# from datos import user_repo

def verificar_seguimiento(follower_id, followed_id):
    return user_repo.comprobar_si_sigue(follower_id, followed_id)

def gestionar_seguimiento(follower_id, followed_id, estado_actual):
    return user_repo.alternar_seguir(follower_id, followed_id, estado_actual)