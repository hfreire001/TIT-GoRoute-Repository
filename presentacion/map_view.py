...
def cargar_tags():
    """Lee el archivo tags.json y devuelve la lista de etiquetas."""
    ruta_json = 'tags.json'
...
def render():
    ...
    if st.session_state['ruta_calculada']:
        ...
        # --- CARGA Y SELECCIÓN DE ETIQUETAS ---
        opciones_tags = cargar_tags()
        tags_seleccionados = st.multiselect(
            "Selecciona etiquetas para tu ruta:",
            options=opciones_tags,
            help="Puedes elegir varias categorías para clasificar tu ruta."
        )
        ...
        if st.button("Guardar en mi perfil", type="primary"):
            if not nombre_ruta:
                ...
                # 🚀 Añadido 'tags_seleccionados' a los parámetros
                exito_guardar = route_repo.guardar_ruta(
                    ...
                    imagen_ruta,
                    tags_seleccionados # <--- AHORA SE PASA A LA FUNCIÓN DE GUARDADO
                )
