from datos.db_connection import close, connect
...
def obtener_rutas_feed(user_id):
    ...
    try:
        ...
        # Como tu cursor no es diccionario, lo convertimos manualmente
...
def alternar_like(user_id, route_id, estado_actual):
    ...
    try:
        ...
        # Guardamos cambios usando la conexión del cursor
...
from datos.db_connection import connect, close # Asegúrate de que estas existan
...
def obtener_ruta_especifica(ruta_id, user_id):
    ...
    dict_cursor = None  # <--- IMPORTANTE: Inicializamos aquí
    ...
    try:
        ...
        # Solo cerramos si se llegó a crear
        if dict_cursor:
            dict_cursor.close()
        close(conn)