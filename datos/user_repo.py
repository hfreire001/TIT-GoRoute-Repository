...
def buscar_usuarios_por_nombre(nombre_buscado):
    """Busca en la tabla users para el buscador del frontend."""
    ...
    try:
        with conexion.cursor() as cursor:
            # Traemos id, username, avatar_url y bio (lo que usa el frontend)
            query = "SELECT id, username, avatar_url, bio FROM users WHERE username ILIKE %s"
            # ILIKE y % para que funcione como buscador real
            cursor.execute(query, (f"%{nombre_buscado}%",))
            return cursor.fetchall() 
            
    except Exception as e:
        print(f"❌ Error buscando usuarios en DB: {e}")
        return []
    finally:
...
def get_favoritos_usuario(user_id):
    """Devuelve las rutas que el usuario ha guardado en favoritos."""
    conexion = get_connection()
    if not conexion: return []
    
    try:
        with conexion.cursor() as cursor:
            # Quitamos el f.created_at y ordenamos por el ID de la tabla rutas
            query = """
                SELECT 
                    r.id, r.name as nombre, r.description,
                    r.thumbnail_url as miniatura, r.creator_id,
                    (SELECT COUNT(*) FROM likes WHERE route_id = r.id) as likes,
                    (SELECT COUNT(*) FROM comments WHERE route_id = r.id) as comentarios,
                    (SELECT COUNT(*) FROM favorites WHERE route_id = r.id) as guardados
                FROM routes r
        ...
        print(f"❌ FALLO SQL EN FAVORITOS: {e}")
        return []
    finally:
        release_connection(conexion)

def comprobar_si_sigue(follower_id, followed_id):
    """Comprueba si el usuario logueado ya sigue a este perfil."""
    conexion = get_connection()
    if not conexion: return False
    
    try:
        with conexion.cursor() as cursor:
            cursor.execute("SELECT 1 FROM follows WHERE follower_id = %s AND followed_id = %s", (follower_id, followed_id))
            return cursor.fetchone() is not None
    except Exception as e:
        print(f"❌ Error al comprobar seguimiento: {e}")
        return False
    finally:
        release_connection(conexion)

def alternar_seguir(follower_id, followed_id, estado_actual):
    """Añade o quita el follow dependiendo del estado actual."""
    conexion = get_connection()
    if not conexion: return False
